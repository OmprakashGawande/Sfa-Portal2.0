...
public partial class mis_Finance_VoucherJournal : System.Web.UI.Page
{
    ...
    //CostCentreDetail Event & Function
            ...
            if (msg == "")
            {
                ...
                if (ViewState["Office_ID"].ToString() == "1")
                {
                    ds11 = objdb.ByProcedure("SpFinVoucherTx",
                       ...
                       new string[] { "29", Voucher_NO, ViewState["VoucherTx_ID"].ToString(), "Journal" }, "dataset");
                }
                else
                {
                   ds11 = objdb.ByProcedure("SpFinVoucherTx",
                            ...
                            for (int j = 0; j < dsBillByBill.Tables.Count; j++)
                            {
                                if (dsBillByBill.Tables[j].TableName == RowNo.ToString())
                                {
                                    for (int k = 0; k < dsBillByBill.Tables[j].Rows.Count; k++)
                                    {
                                        ...
                                        new string[] { "1", VoucherTx_ID, Ledger_ID, "Journal", ChequeTx_No, ChequeTx_Date, ChequeTx_Amount, Month.ToString(), Year.ToString(), FinancialYear.ToString(), ViewState["Office_ID"].ToString(), "0", ViewState["Emp_ID"].ToString(), (k + 1).ToString(), RowNo.ToString(), IfCheque, AcPayee, FavouringName }, "dataset");
                                    }
                                }
                            }
                        }
                            ...
                            for (int j = 0; j < dsBillByBill.Tables.Count; j++)
                            {
                                if (dsBillByBill.Tables[j].TableName == RowNo.ToString())
                                {
                                    for (int k = 0; k < dsBillByBill.Tables[j].Rows.Count; k++)
                                    {
                                        ...
                                        objdb.ByProcedure("SpFinChequeTx",
                                        new string[] { "flag", "VoucherTx_ID", "Ledger_ID", "VoucherTx_Type", "ChequeTx_No", "ChequeTx_Date", "ChequeTx_Amount", "ChequeTx_Month", "ChequeTx_Year", "ChequeTx_FY", "Office_ID", "ChequeTx_IsActive", "ChequeTx_InsertedBy", "ChequeTx_OrderBy", "LedgerTx_OrderBy", "IfCheque", "AcPayee", "FavouringName" },
                                        new string[] { "1", ViewState["VoucherTx_ID"].ToString(), Ledger_ID, "Journal", ChequeTx_No, ChequeTx_Date, ChequeTx_Amount, Month.ToString(), Year.ToString(), FinancialYear.ToString(), ViewState["Office_ID"].ToString(), "1", ViewState["Emp_ID"].ToString(), (k + 1).ToString(), RowNo.ToString(), IfCheque, AcPayee, FavouringName }, "dataset");
                                    }
                                }
                            }
                        }
