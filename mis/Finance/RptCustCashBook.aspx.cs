...
public partial class mis_Finance_RptCustCashBook : System.Web.UI.Page
{
    DataSet ds;
    ...
    protected void FillGrid()
    {
        try
        {
            ...
            txtAllBankOpening.Text = "";
            txtAllBankClosing.Text = "";
            ...
            ds = objdb.ByProcedure("SpFinCustCashBook", new string[] { "Office_ID_Mlt", "FromDate", "ToDate", "FinancialYear" }, new string[] { Office, Convert.ToDateTime(txtFromDate.Text, cult).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtToDate.Text, cult).ToString("yyyy/MM/dd"), FinancialYear }, "dataset");
            if (ds.Tables.Count != 0 && ds.Tables.Count != 0)
            {

                string headingFirst = "<p class='text-center' style='font-weight:600; text-align:center'> Custom Cash Book<br />SFA Technologies Pvt. Ltd. <br/> [ " + ddlOffice.SelectedItem.Text + " ] <br />  " + Convert.ToDateTime(txtFromDate.Text, cult).ToString("dd-MM-yyyy") + "  To " + Convert.ToDateTime(txtToDate.Text, cult).ToString("dd-MM-yyyy") + "</p>";
                ...
                if (ds.Tables[1].Rows.Count > 0)
                {
                    ...
                    OpeningBalance = decimal.Parse(ds.Tables[1].Rows[0]["PreOpeningBalance"].ToString());
                    ClosingBalance = decimal.Parse(ds.Tables[1].Rows[0]["OpeningBalance"].ToString());
                    if (OpeningBalance < 0)
                    {
                        OpeningType = "Dr";
                    }
                    ...
                    txtAllBankOpening.Text = "<span style='color:red; font-size:17px;'>Bank Opening (All) :</span>     " + Math.Abs(OpeningBalance).ToString() + " " + OpeningType;
                    txtAllBankClosing.Text = "<span style='color:red; font-size:17px;'>Bank Closing (All) :</span>     " + Math.Abs(ClosingBalance).ToString() + " " + ClosingType;
