                        ...
                        if (ds != null && ds.Tables[0].Rows.Count > 0)
                        {
                            if (ds.Tables[0].Rows[0]["InventoryAffected"].ToString() == "Yes")
                            {
                                objdb.ByProcedure("SpFinItemTx",
                                   ...
                                   , new string[] { "4", lblItemID.Text, lblQuantity.Text, "0", lblRate.Text, VoucherTx_ID, "Goods Purchase Tax Free", VoucherTx_No, ViewState["Office_ID"].ToString(), lblWarehouse_id.Text, ViewState["Emp_ID"].ToString(), Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd"),lblAmount.Text }, "dataset");
                            }
                        }
                ...
                for (int rowIndex = 0; rowIndex < gridRows; rowIndex++)
                {
                    ...
                    if (ds != null && ds.Tables[0].Rows.Count > 0)
                    {
                        if (ds.Tables[0].Rows[0]["InventoryAffected"].ToString() == "Yes")
                        {
                            objdb.ByProcedure("SpFinItemTx",
                                new string[] { "flag", "Item_id", "Cr", "Dr", "Rate", "TransactionID", "TransactionFrom", "InvoiceNo", "Office_Id", "Warehouse_id", "CreatedBy", "TranDt", "Amount" }
                               , new string[] { "4", lblItemID.Text, lblQuantity.Text, "0", lblRate.Text, ViewState["VoucherTx_ID"].ToString(), "Goods Purchase Tax Free", VoucherTx_No, ViewState["Office_ID"].ToString(), lblWarehouse_id.Text, ViewState["Emp_ID"].ToString(), Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd"),lblAmount.Text }, "dataset");
                        }
                    }
                    string Amount = lblAmount.Text;
                    Amount = "-" + Amount;
