...
public partial class mis_Finance_RptExpensesAssets : System.Web.UI.Page
{
    DataSet ds;
    ...
    protected void FillGrid()
    {
        try
        {
            ...
            if (Office != "")
            {
                ds = objdb.ByProcedure("SpFinExpensesAssetsReport", new string[] { "Office_ID_mlt", "FromDate", "ToDate", "OpeningFromDate" }, new string[] { Office, Convert.ToDateTime(txtFromDate.Text, cult).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtToDate.Text, cult).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtFromDate.Text, cult).ToString("yyyy/MM/dd") }, "dataset");
                if (ds.Tables.Count != 0 && ds.Tables[0].Rows.Count != 0)
                {
                    //divshow.Visible = true;
                    string headingFirst = "<p class='text-center' style='font-weight:600; text-align:center'> SFA Technologies Pvt. Ltd.<br/>खेल परिसर , इंदिरा निकुंज नर्सरी, 74 बंगले,भोपाल  <br/>" + FinancialYear.ToString() + "  <br />" + Convert.ToDateTime(txtFromDate.Text, cult).ToString("dd-MM-yyyy") + "  To " + Convert.ToDateTime(txtToDate.Text, cult).ToString("dd-MM-yyyy") + "भुगतान का विवरण</p>";
                    ...
                    sb.Append("<th>विवरण</th>");
                    sb.Append("<th>बजट आवंटन</th>");
                    sb.Append("<th>गत माह तक व्यय</th>");
                    sb.Append("<th>वर्तमान माह में व्यय</th>");
                    sb.Append("<th>कुल व्यय शेष</th>");
                    sb.Append("</tr>");
                    sb.Append("</thead>");
                    ...
                    for (int i = 0; i < Count; i++)
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            sb.Append("<td>" + ds.Tables[0].Rows[i]["Budget_Amount"].ToString() + "</td>");
    ...
    //        Response.Clear();
    //        Response.AddHeader("content-disposition", "attachment;filename=" + "व्यय के मद" + DateTime.Now + ".xls");
    //        Response.Charset = "";
    ...
    protected void FillGridTesting()
    {
        try
        {
            ...
            if (Office != "")
            {
                ...
                ds = objdb.ByProcedure("SpFinExpensesAssetsReport_New", new string[] { "Office_ID_mlt", "FromDate", "ToDate", "OpeningFromDate", "FinancialYear" }, new string[] { Office, Convert.ToDateTime(txtFromDate.Text, cult).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtToDate.Text, cult).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtFromDate.Text, cult).ToString("yyyy/MM/dd"), FinancialYear }, "dataset");
                if (ds.Tables.Count != 0 && ds.Tables[0].Rows.Count != 0)
                {
                    //divshow.Visible = true;
                    ...
                    DataTable dt = new DataTable();
                    dt.Columns.Add("ParentHeadCode", typeof(string));
                    dt.Columns.Add("Parent_Head_Name_Hindi", typeof(string));
                    ...
                    dt.Columns.Add("Budget_Allocation", typeof(string));
                    ...
                    for (int i = 0; i < Count; i++)
                    {
                        //decimal Total = Math.Abs((Convert.ToDecimal(ds.Tables[0].Rows[i]["PreFromOpeningBalance"].ToString()) + Convert.ToDecimal(ds.Tables[0].Rows[i]["CurrentTxn"].ToString())));
                        decimal Total = (Convert.ToDecimal(ds.Tables[0].Rows[i]["PreFromOpeningBalance"].ToString()) + Convert.ToDecimal(ds.Tables[0].Rows[i]["CurrentTxn"].ToString()));
                        decimal Balance = (Convert.ToDecimal(ds.Tables[0].Rows[i]["Budget_Amount"].ToString()) - Math.Abs(Total));
                        if (i == 0)
                        {
                            dt.Rows.Add(ds.Tables[0].Rows[i]["ParentHeadCode"].ToString(),
                                        ds.Tables[0].Rows[i]["Parent_Head_Name_Hindi"].ToString(),
                                        ds.Tables[0].Rows[i]["Head_Code"].ToString(),
                                        ...
                                        Math.Abs(Convert.ToDecimal(ds.Tables[0].Rows[i]["Budget_Amount"].ToString())),
                            ...
                            Budget_Total += Convert.ToDecimal(ds.Tables[0].Rows[i]["Budget_Amount"].ToString());
                            PreFromOpeningBalance_Total += Convert.ToDecimal(ds.Tables[0].Rows[i]["PreFromOpeningBalance"].ToString());
                            CurrentTxn_Total += Convert.ToDecimal(ds.Tables[0].Rows[i]["CurrentTxn"].ToString());
                            T_Total += Total;
                            Bal_Total += Balance; 
                        }
                        else if (ParentHeadName == ds.Tables[0].Rows[i]["Parent_Head_Name_Hindi"].ToString())
                        {
                            if(HeadName == ds.Tables[0].Rows[i]["Head_Name"].ToString())
                            {
                                dt.Rows.Add("",
                                            "",
                                            "",
                        ...
                        else
                        {
                            dt.Rows.Add("",
                                        ...
                                        "",
                                        "योग",
                                        Math.Abs(Budget_Total).ToString(),
                        ...
                        HeadName = ds.Tables[0].Rows[i]["Head_Name"].ToString();
                        ParentHeadName = ds.Tables[0].Rows[i]["Parent_Head_Name_Hindi"].ToString();
                    ...
                    decimal Budget_Total_TotalSum = ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("Budget_Amount"));
                    decimal TotalPreFromOpeningBalance_TotalSum = ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("PreFromOpeningBalance"));
                    decimal CurrentTxn_TotalSum = ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("CurrentTxn"));
                    decimal TTotalSum = (TotalPreFromOpeningBalance_TotalSum + CurrentTxn_TotalSum);
                    decimal BalTotalSum = (Budget_Total_TotalSum - TTotalSum);
                    dt.Rows.Add("",
                                "",
                                "",
                                "",
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            //decimal Val = decimal.Parse(e.Row.Cells[10].Text.ToString());
