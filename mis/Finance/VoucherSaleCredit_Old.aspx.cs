...
public partial class mis_Finance_VoucherSaleCredit_Old : System.Web.UI.Page
{
    DataSet ds;
    ...
    protected void FillVoucherNo()
    {
        try
        {
            ...
            //start done changes by Mohini on 13April2022
            string[] FinY = FinancialYear.Split('-');
            ViewState["CGST"] = "1";
            ViewState["SGST"] = "2";
            //if (int.Parse(FinY[0]) >= 2022)
            //{
            //    ViewState["CGST"] = "46721";
            //    ViewState["SGST"] = "46722";
            ...
            //    ViewState["SGST"] = "2";
            //}
            //end done changes by Mohini on 13April2022
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        {
            string msg = "";
            HF_SubItemTotal.Value = "";
            lblSubItemTotal.Text = "";
            ...
            if (msg == "")
            {
                ...
                if (ds.Tables[0].Rows.Count != 0)
                {
                    ...
                    lblsubItemDifference.Text = txtTotalAmount.Text;
                    ...
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "ModalSubItemDetail('0');", true);
                }
                else
                {
                    btnAdd_Click_Action();
    ...
    protected void AddItem(string ID)
    {
        try
        {
            ...
            dt_GridViewItem.Columns.Add(new DataColumn("ItemID", typeof(string)));
	        dt_GridViewItem.Columns.Add(new DataColumn("ItemDescription", typeof(string)));
            dt_GridViewItem.Columns.Add(new DataColumn("Unit_id", typeof(string)));
            dt_GridViewItem.Columns.Add(new DataColumn("Warehouse_id", typeof(string)));
            dt_GridViewItem.Columns.Add(new DataColumn("WarehouseName", typeof(string)));
            ...
            for (rowIndex = 0; rowIndex < gridRows; rowIndex++)
            {
		        ...
		        Label lblItemDescription = (Label)GridViewItem.Rows[rowIndex].Cells[0].FindControl("lblItemDescription");
                ...
                {
                    RID++;
                    dt_GridViewItem.Rows.Add(lblItemRowNo.Text, lblItemID.Text,lblItemDescription.Text, lblUnit_id.Text, lblWarehouse_id.Text, lblWarehouseName.Text, lblHSNCode.Text, lblItem.Text, lblQuantity.Text, lblRate.Text, lblAmount.Text, lblCGST.Text, lblSGST.Text, lblIGST.Text, lblCGSTPer.Text, lblSGSTPer.Text, lblIGSTPer.Text, lblUnit.Text, lblLedgerName.Text, lblLedgerID.Text, lblTaxbility.Text);
                }
            ...
            if (gridRows > 2)
            {
                for (rowIndex = 0; rowIndex < gridRows; rowIndex++)
                {
                    ...
                    if (lblID.Text == "1" || lblID.Text == "2" || lblID.Text == "3" || lblID.Text == "46721" || lblID.Text == "46722")
                    {
                        status = "0";
                    }
                    else
                ...
                if (dt_GridViewItem.Rows.Count > 0)
                {
                    ...
                    if (ViewState["CGST"].ToString() == "46721")
                    {
                        dt_GridViewLedger.Rows.Add(ViewState["CGST"].ToString(), "CGST OUTPUT", CGST.ToString(), HSN_Code, CGST_Per, SGST_Per, IGST_Per, CGSTAmt, SGSTAmt, IGSTAmt, status, null, null);
                    }
                    if (ViewState["SGST"].ToString() == "2")
                    {
                    ...
                    if (ViewState["SGST"].ToString() == "46722")
                    {
                        dt_GridViewLedger.Rows.Add(ViewState["SGST"].ToString(), "SGST OUTPUT", SGST.ToString(), HSN_Code, CGST_Per, SGST_Per, IGST_Per, CGSTAmt, SGSTAmt, IGSTAmt, status, null, null);
                    }
                ...
                else
                {
                    ...
                    if (ViewState["CGST"].ToString() == "46721")
                    {
                        dt_GridViewLedger.Rows.Add(ViewState["CGST"].ToString(), "CGST OUTPUT", CGST, HSN_Code, CGST_Per, SGST_Per, IGST_Per, CGSTAmt, SGSTAmt, IGSTAmt, status, null, null);
                    }
                    if (ViewState["SGST"].ToString() == "2")
                    {
                    ...
                    if (ViewState["SGST"].ToString() == "46722")
                    {
                        dt_GridViewLedger.Rows.Add(ViewState["SGST"].ToString(), "SGST OUTPUT", SGST, HSN_Code, CGST_Per, SGST_Per, IGST_Per, CGSTAmt, SGSTAmt, IGSTAmt, status, null, null);
                    }
                ...
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        for (int rowIndex = 0; rowIndex < gridRows; rowIndex++)
                        {
                            ...
                            Label lblGSTApplicable = (Label)GridViewLedger.Rows[rowIndex].Cells[0].FindControl("lblGSTApplicable");
                            Label lblTaxbility = (Label)GridViewLedger.Rows[rowIndex].Cells[0].FindControl("lblTaxbility");
                            if (lblID.Text == "1" || lblID.Text == "2" || lblID.Text == "3" || lblID.Text == "737" || lblID.Text == "46721" || lblID.Text == "46722")
                            {
                                status = "0";
                            }
                            else
    ...
    protected void btnAccept_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //}
            if (GridViewDebtor.Rows.Count == 0)
            {
                msg += "Enter Debtor Detail. \\n";
            }
            ...
            if (msg == "")
            {
			    ...
			    Label lblItemDescription = (Label)GridViewItem.Rows[rowItemIndex].Cells[0].FindControl("lblItemDescription");
                            ...
                            objdb.ByProcedure("SpFinItemTx", new string[] { "flag", "VoucherTx_ID", "VoucherTx_Name", "VoucherTx_Type", "Ledger_ID", "Item_id","ItemDesc", "Unit_id", "Quantity", "Rate", "Amount", "HSN_Code", "IGST_Per", "CGST_Per", "SGST_Per", "CGSTAmt", "SGSTAmt", "IGSTAmt", "Warehouse_id", "Office_ID", "ItemTx_FY", "ItemTx_IsActive", "ItemTx_InsertedBy", "ItemTx_OrderBy", "Taxbility" }
                                , new string[] { "0", VoucherTx_ID, "Sales Voucher", "CreditSale Voucher", lblLedgerID.Text, lblItemID.Text,lblItemDescription.Text, lblUnit_id.Text, lblQuantity.Text, lblRate.Text, lblAmount.Text, lblHSNCode.Text, lblIGSTPer.Text, lblCGSTPer.Text, lblSGSTPer.Text, lblCGST.Text, lblSGST.Text, lblIGST.Text, ddlWarehouse.SelectedValue.ToString(), ViewState["Office_ID"].ToString(), FinancialYear.ToString(), ItemTx_IsActive, ViewState["Emp_ID"].ToString(), lblItemRowNo.Text, lblTaxbility.Text }, "dataset");
                        ...
                        string url = "VoucherSalepurchaseInvocieQR.aspx?VoucherTx_ID=" + objdb.Encrypt(VoucherTx_ID).ToString();
			...
			Label lblItemDescription = (Label)GridViewItem.Rows[rowItemIndex].Cells[0].FindControl("lblItemDescription");
                        ...
                        Label lblLedgerID = (Label)GridViewItem.Rows[rowItemIndex].Cells[0].FindControl("lblLedgerID");
                        Label lblTaxbility = (Label)GridViewItem.Rows[rowItemIndex].Cells[0].FindControl("lblTaxbility");
                        objdb.ByProcedure("SpFinItemTx", new string[] { "flag", "VoucherTx_ID", "VoucherTx_Name", "VoucherTx_Type", "Ledger_ID", "Item_id","ItemDesc", "Unit_id", "Quantity", "Rate", "Amount", "HSN_Code", "IGST_Per", "CGST_Per", "SGST_Per", "CGSTAmt", "SGSTAmt", "IGSTAmt", "Warehouse_id", "Office_ID", "ItemTx_FY", "ItemTx_IsActive", "ItemTx_InsertedBy", "ItemTx_OrderBy", "Taxbility" }, new string[] { "0", ViewState["VoucherTx_ID"].ToString(), "Sales Voucher", "CreditSale Voucher", lblLedgerID.Text, lblItemID.Text,lblItemDescription.Text,  lblUnit_id.Text, lblQuantity.Text, lblRate.Text, lblAmount.Text, lblHSNCode.Text, lblIGSTPer.Text, lblCGSTPer.Text, lblSGSTPer.Text, lblCGST.Text, lblSGST.Text, lblIGST.Text, ddlWarehouse.SelectedValue.ToString(), ViewState["Office_ID"].ToString(), FinancialYear.ToString(), ItemTx_IsActive, ViewState["Emp_ID"].ToString(), lblItemRowNo.Text, lblTaxbility.Text }, "dataset");
                        ds = objdb.ByProcedure("SpFinItemTx", new string[] { "flag", "Ledger_ID" }, new string[] { "3", lblLedgerID.Text }, "dataset");
                        if (ds != null && ds.Tables[0].Rows.Count > 0)
                        {
                            if (ds.Tables[0].Rows[0]["InventoryAffected"].ToString() == "Yes")
                    ...
                    lblMsg.Text = objdb.Alert("fa-check", "alert-success", "Thank you!", "Operation Completed Successfully.");
                    string url = "VoucherSalepurchaseInvocieQR.aspx?VoucherTx_ID=" + objdb.Encrypt(ViewState["VoucherTx_ID"].ToString()).ToString();
                    StringBuilder sb = new StringBuilder();
                    sb.Append("<script type = 'text/javascript'>");
                    sb.Append("window.open('");
                    sb.Append(url);
        ...
        try
        {
            ...
            if (ds != null)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    ddlCategory.Items.FindByValue(ds.Tables[0].Rows[0]["Category"].ToString()).Selected = true;
                    txtFarmerOtherDetail.Text = ds.Tables[0].Rows[0]["FarmerOtherDetail"].ToString();
                ...
                if (ds.Tables[8].Rows.Count > 0)
                {
                    ...
                    int rowscount = ds.Tables[8].Rows.Count;
                    for (int i = 0; i < rowscount; i++)
                    {

                        string TNO = ds.Tables[8].Rows[i]["Item_id"].ToString();
                        string Ingredient_ID = ds.Tables[8].Rows[i]["Ingredient_id"].ToString();
                        string SubItem_Quantity = ds.Tables[8].Rows[i]["Quantity"].ToString();
                        string SubItem_Unit = ds.Tables[8].Rows[i]["ItemTx_Unit"].ToString();
                        string SubItem_Rate = ds.Tables[8].Rows[i]["Rate"].ToString();
                        string SubItem_Amount = ds.Tables[8].Rows[i]["Amount"].ToString();
                        string Ingredient_Name = ds.Tables[8].Rows[i]["Ingredient_Name"].ToString();
                        string Ingredient_Size = ds.Tables[8].Rows[i]["Ingredient_Size"].ToString();

                        dt_GridViewItem_Ingredient.Rows.Add(TNO, Ingredient_ID, SubItem_Quantity, SubItem_Unit, SubItem_Rate, SubItem_Amount, Ingredient_Name, Ingredient_Size);
    ...
    protected void CreateTableFinSubItem()
    {
        try
        {
            ...
            dt_GridViewItem_Ingredient.Columns.Add(new DataColumn("Ingredient_Name", typeof(string)));
            dt_GridViewItem_Ingredient.Columns.Add(new DataColumn("Ingredient_Size", typeof(string)));
    ...
    protected void btnAddSubItem_Click(object sender, EventArgs e)
    {
        try
        {
            double SubItem_amount = 0.00;
            double SubItemTotal = 0.00;
            if (lblSubItem_amount.Text != "")
                SubItem_amount = double.Parse(lblSubItem_amount.Text);


            if (HF_SubItemTotal.Value != "")
                SubItemTotal = double.Parse(HF_SubItemTotal.Value);

            if (SubItem_amount == SubItemTotal)
            {
                AddSubItem();
                btnAdd_Click_Action();
            }
            else
            {
                ...
                Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "ModalSubItemDetail('1');", true);
            }
    ...
    private void AddSubItem()
    {
        try
        {
            ...
            foreach (GridViewRow row in GVFinItemIngredientTx.Rows)
            {
                ...
                TextBox Ingredient_Size = (TextBox)row.FindControl("txtIngredient_Size");

                if (chkSelect.Checked == true)
                {
                    dt_GridViewItem_Ingredient.Rows.Add(TNO, Ingredient_ID, SubItem_Quantity.Text, SubItem_Unit.Text, SubItem_Rate.Text, SubItem_Amount.Text, Ingredient_Name.Text, Ingredient_Size.Text);
                }
            }
            ViewState["dt_GridViewItem_Ingredient"] = dt_GridViewItem_Ingredient;
    ...
    private void DeleteSubItem(string itemid)
    {
        try
        {
            ...
            newdt_GridViewItem_Ingredient.Columns.Add(new DataColumn("Ingredient_Name", typeof(string)));
            newdt_GridViewItem_Ingredient.Columns.Add(new DataColumn("Ingredient_Size", typeof(string)));
            ...
            foreach (DataRow row in dt_GridViewItem_Ingredient.Rows)
            {
                ...
                string Ingredient_Name = row["Ingredient_Name"].ToString();
                string Ingredient_Size = row["Ingredient_Size"].ToString();

                if (TNO.ToString() != itemid.Trim())
                {
                    newdt_GridViewItem_Ingredient.Rows.Add(TNO, Ingredient_ID, SubItem_Quantity, SubItem_Unit, SubItem_Rate, SubItem_Amount, Ingredient_Name, Ingredient_Size);
                }
            }
    ...
    private void ViewSubItem(string itemid)
    {
        try
        {
            DataTable newdt_GridViewItem_Ingredient = new DataTable();
            ...
            GVFinViewItemIngredientTx.DataSource = newdt_GridViewItem_Ingredient;
            GVFinViewItemIngredientTx.DataBind();

            Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "ModalViewSubItemDetail();", true);
        }
        catch (Exception ex)
        {
            lblMsg.Text = objdb.Alert("fa-ban", "alert-danger", "Sorry!", ex.Message.ToString());
    ...
    private void InsertSubItemDB(string VoucherTx_ID, string VoucherTx_Name, string VoucherTx_Type, string Edit)
    {
        try
        {
            objdb.ByProcedure("SpFinItemIngredientTx", new string[] { "flag", "VoucherTx_ID" }, new string[] { "12", VoucherTx_ID.ToString() }, "dataset");
            ...
            foreach (DataRow row in dt_GridViewItem_Ingredient.Rows)
            {
                ...
                objdb.ByProcedure("SpFinItemIngredientTx",
                          new string[] { "flag", "VoucherTx_ID", "VoucherTx_Name", "VoucherTx_Type", "Item_id", "Ingredient_id", "Quantity", "Rate", "Amount", "Office_ID", "ItemTx_IsActive", "ItemTx_InsertedBy", "ItemTx_Unit", "Ingredient_Name", "Ingredient_Size" },
                          new string[] { "0", VoucherTx_ID.ToString(), VoucherTx_Name.ToString(), VoucherTx_Type.ToString(), item_id.ToString(), Ingredient_id.ToString(), Quantity.ToString(), Rate.ToString(), Amount.ToString(), ViewState["Office_ID"].ToString(), "1", ViewState["Emp_ID"].ToString(), Unit.ToString(), Ingredient_Name, Ingredient_Size }
                          , "dataset");
            ...
            if (txtVoucherTx_OrderDate.Text != "")
            {
                OrderDate = Convert.ToDateTime(OrderDate, cult).ToString("yyyy/MM/dd");
            }
            ...
            if (Edit == "0")
            {
                 objdb.ByProcedure("SpFinItemIngredientTx",
                              ...
                              new string[] { "2", VoucherTx_ID.ToString(), Gram.ToString(), SADO.ToString(), OrderDate, OrderNo, scheme.ToString(), regNo.ToString(), Advance_Cr_Type.ToString(), Advance_Cr_Details, advanceAmount.ToString(), Farmer }
                              , "dataset");
            ...
            else if (Edit == "1")
            {
                 objdb.ByProcedure("SpFinItemIngredientTx",
                              new string[] { "flag", "VoucherTx_ID", "Gram", "Sado", "OrderDate", "OrderNo", "OrderScheme", "RegNo", "Advance_Cr_Type", "Advance_Cr_Details", "AdvanceAmount", "Farmer" },
                              new string[] { "11", VoucherTx_ID.ToString(), Gram.ToString(), SADO.ToString(), OrderDate, OrderNo, scheme.ToString(), regNo.ToString(), Advance_Cr_Type.ToString(), Advance_Cr_Details, advanceAmount.ToString(), Farmer }
                              , "dataset");
    ...
    {
        string Item_ID = GridViewItem.SelectedDataKey.Values["ItemID"].ToString();
        ViewSubItem(Item_ID);
        Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "ModalViewSubItemDetail();", true);
    }
