...
public partial class mis_Finance_RptMonthlyCustCashBookHeadWise : System.Web.UI.Page
{
    DataSet ds;
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        ...
        //sb1.Append("<p class='text-center' style='font-weight:600; font-size:16px; text-align:center'> Custom Cash Book<br /> MP State Agro Industries Development Corporation, <br/> [ " + ddlOffice.SelectedItem.Text + " ] <br />  " + Convert.ToDateTime(txtFromDate.Text, cult).ToString("dd-MM-yyyy") + "  To " + Convert.ToDateTime(txtToDate.Text, cult).ToString("dd-MM-yyyy") + "</span>");
    ...
    protected void FillGrid()
    {
        try
        {
            ...
            ds = objdb.ByProcedure("SpFinMonthlyCustCashBookHeadWise", new string[] { "Office_ID_Mlt", "FromDate", "ToDate", "FinancialYear" }, new string[] { Office, Convert.ToDateTime(txtFromDate.Text, cult).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtToDate.Text, cult).ToString("yyyy/MM/dd"), FinancialYear }, "dataset");
            if (ds.Tables.Count != 0 && ds.Tables.Count != 0)
            {
                
                string headingFirst = "<p class='text-center' style='font-weight:600; text-align:center'>Head Wise Cash Book<br /> SFA Technologies Pvt. Ltd. <br/> [ " + ddlOffice.SelectedItem.Text + " ] <br />  " + Convert.ToDateTime(txtFromDate.Text, cult).ToString("dd-MM-yyyy") + "  To " + Convert.ToDateTime(txtToDate.Text, cult).ToString("dd-MM-yyyy") + "</span>";
                lblheadingFirst.Text = headingFirst;
                lblReceiptHeading.Text = "<p class='text-center' style='font-weight:600; text-align:center'>प्राप्ति विवरण</span>";
                lblPaymentHeading.Text = "<p class='text-center' style='font-weight:600; text-align:center'>भुगतान विवरण</span>";
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
						Label lblLedger_Name = (Label)rows.FindControl("lblLedger_Name");
                        DataTable dt = new DataTable();
                        StringBuilder sb = new StringBuilder();
                        HiddenField HF_VoucherTx_ID = (HiddenField)rows.FindControl("HF_VoucherTx_ID");
                        System.Web.UI.HtmlControls.HtmlGenericControl divcostcentre = (System.Web.UI.HtmlControls.HtmlGenericControl)rows.FindControl("divcostcentre");
                        ds.Tables[3].DefaultView.RowFilter = "VoucherTx_ID = '" + HF_VoucherTx_ID.Value + "'";
                        dt = ds.Tables[3].DefaultView.ToTable();
                        ...
                        if(dtCount > 0)
                        {
                            for (int i = 0; i < dtCount; i++)
                            {
                                ...
                                {
                                    // sb.Append("<tr>");
                                    sb.Append("<span>[" + dt.Rows[i]["Ledger_Code"].ToString() + "]   " + dt.Rows[i]["Ledger_Name_Hindi"].ToString() + "</span>");
                                    sb.Append("</br>");
                                    // sb.Append("<tr>");
                                    sb.Append("<span style='padding-left:0px;'>" + dt.Rows[i]["CategoryName"].ToString() + "</span>");
                                    sb.Append("</br>");
                                    //sb.Append("<tr>");
                                    sb.Append("<span style='float:left; padding-left:0px;' class='subcategory'>" + dt.Rows[i]["SubCategoryName"].ToString() + "</span><span style='float:right' class='subcategory'>" + dt.Rows[i]["Amount"].ToString() + "</span>");
                                    sb.Append("</br>");
                                    //sb.Append("<td>" + dt.Rows[i]["Amount"].ToString() + "</td>");
                                    //sb.Append("</tr>");
                                }
                                else if (Ledger_ID == dt.Rows[i]["Ledger_ID"].ToString())
                                {

                                    if (CategoryId == dt.Rows[i]["CategoryId"].ToString())
                                    {

                                        //sb.Append("<tr>");
                                        //sb.Append("<td colspan='2'>" + dt.Rows[i]["SubCategoryName"].ToString() + "</td>");
                                        //sb.Append("<td>" + dt.Rows[i]["Amount"].ToString() + "</td>");
                                        //sb.Append("</tr>");
                                        sb.Append("<span style='float:left; padding-left:0px;' class='subcategory'>" + dt.Rows[i]["SubCategoryName"].ToString() + "</span><span style='float:right' class='subcategory'>" + dt.Rows[i]["Amount"].ToString() + "</span>");
                                        sb.Append("</br>");
                                    ...
                                    else
                                    {
                                        ...
                                        sb.Append("</br>");
                                        //sb.Append("<tr>");
                                        //sb.Append("<td colspan='1'>" + dt.Rows[i]["CategoryName"].ToString() + "</td>");
                                        //sb.Append("</tr>");
                                        //sb.Append("<tr>");
                                        //sb.Append("<td>" + dt.Rows[i]["SubCategoryName"].ToString() + "</td>");
                                        //sb.Append("<td>" + dt.Rows[i]["Amount"].ToString() + "</td>");
                                        //sb.Append("</tr>");
                                ...
                                else
                                {
                                    ...
                                    // sb.Append("<tr>");
                                    sb.Append("<span style='padding-top:5px;'>" + dt.Rows[i]["CategoryName"].ToString() + "</span>");
                                    sb.Append("</br>");
                                    //sb.Append("<tr>");
                                    sb.Append("<span style='float:left; padding-left:0px;' class='subcategory'>" + dt.Rows[i]["SubCategoryName"].ToString() + "</span><span style='float:right' class='subcategory'>" + dt.Rows[i]["Amount"].ToString() + "</span>");
                                    sb.Append("</br>");
                                    //sb.Append("<tr>");
                                    //sb.Append("<td>" + dt.Rows[i]["Ledger_Code"].ToString() + "</td>");
                                    //sb.Append("</tr>");
                                    //sb.Append("<tr>");
                                ...
                                // sb.Append("</table");
                                divcostcentre.InnerHtml = sb.ToString();
                            }
                          
                        }
                        else
                        {
                             sb.Append("<span>"+lblLedger_Name.Text+"</span>");
                             divcostcentre.InnerHtml = sb.ToString();
                        }
                    }
                }
                if (ds.Tables[1].Rows.Count > 0)
                {
                    ...
                    foreach (GridViewRow rows in GvPaymentDetail.Rows)
                    {
						...
						Label lblLedger_Name = (Label)rows.FindControl("lblLedger_Name");
                        HiddenField HF_VoucherTx_ID = (HiddenField)rows.FindControl("HF_VoucherTx_ID");
                        System.Web.UI.HtmlControls.HtmlGenericControl divPayCostCentre = (System.Web.UI.HtmlControls.HtmlGenericControl)rows.FindControl("divPayCostCentre");
                        ds.Tables[4].DefaultView.RowFilter = "VoucherTx_ID = '" + HF_VoucherTx_ID.Value + "'";
                        dt = ds.Tables[4].DefaultView.ToTable();
                        int dtCount = dt.Rows.Count;
                        string Ledger_ID = "";
                        string CategoryId = "";
                        if (dtCount > 0)
                            ...
                            for (int i = 0; i < dtCount; i++)
                            {
                                ...
                                else if (Ledger_ID == dt.Rows[i]["Ledger_ID"].ToString())
                                {
                                    ...
                                    if (CategoryId == dt.Rows[i]["CategoryId"].ToString())
                                    {
                                        ...
                                        //sb.Append("<td>" + dt.Rows[i]["Amount"].ToString() + "</td>");
                                        //sb.Append("</tr>");
                                        // sb.Append("<span style='padding-left:10px;'>" + dt.Rows[i]["SubCategoryName"].ToString() + "</span><span style='padding-left:100px;'>" + dt.Rows[i]["Amount"].ToString() + "</span>");
                                        sb.Append("<span style='float:left; padding-left:0px;' class='subcategory'>" + dt.Rows[i]["SubCategoryName"].ToString() + "</span><span style='float:right' class='subcategory'>" + dt.Rows[i]["Amount"].ToString() + "</span>");
                                        sb.Append("</br>");
                                    }
                                    else
                                ...
                                CategoryId = dt.Rows[i]["CategoryId"].ToString();
                                // sb.Append("</table");
                                divPayCostCentre.InnerHtml = sb.ToString();
                            }
                        }
                        else
                        {
    ...
    protected void lnkBankTransactionDetails_Click(object sender, EventArgs e)
    {
        GvPaymentDetail.UseAccessibleHeader = true;
        GvPaymentDetail.HeaderRow.TableSection = TableRowSection.TableHeader;
        GvReceiptDetail.UseAccessibleHeader = true;
        GvReceiptDetail.HeaderRow.TableSection = TableRowSection.TableHeader;
        gvBankTransactionDetails.DataSource = string.Empty;
        gvBankTransactionDetails.DataBind();
        ds = objdb.ByProcedure("SpFinRptCashBankBooksNew", new string[] { "flag", "Office_ID_Mlt", "Head_ID", "FromDate", "ToDate" }, new string[] { "3", ddlOffice.SelectedValue, "117", Convert.ToDateTime(txtFromDate.Text, cult).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtToDate.Text, cult).ToString("yyyy/MM/dd") }, "dataset");
        if (ds.Tables.Count != 0 && ds.Tables[1].Rows.Count != 0)
        {
            gvBankTransactionDetails.DataSource = ds.Tables[1];
            gvBankTransactionDetails.DataBind();
            decimal OpeningTotal = ds.Tables[1].AsEnumerable().Sum(row => row.Field<decimal>("PreOpeningBalance"));
            decimal ClosingTotal = ds.Tables[1].AsEnumerable().Sum(row => row.Field<decimal>("OpeningBalance"));
            decimal TxnDebitAmt = ds.Tables[1].AsEnumerable().Sum(row => row.Field<decimal>("DebitAmt"));
            decimal TxnCreditAmt = ds.Tables[1].AsEnumerable().Sum(row => row.Field<decimal>("CreditAmt"));
            gvBankTransactionDetails.FooterRow.Cells[1].Text = "<b>Total : </b>";
            gvBankTransactionDetails.FooterRow.Cells[2].Text = "<b>" + Math.Abs(OpeningTotal).ToString() + "</b>";
            gvBankTransactionDetails.FooterRow.Cells[3].Text = "<b>" + Math.Abs(TxnDebitAmt).ToString() + "</b>";
            gvBankTransactionDetails.FooterRow.Cells[4].Text = "<b>" + Math.Abs(TxnCreditAmt).ToString() + "</b>";
            gvBankTransactionDetails.FooterRow.Cells[5].Text = "<b>" + Math.Abs(ClosingTotal).ToString() + "</b>";
            Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "ShowBankTransactionModal();", true);
        }
    }
}