...
public partial class mis_Finance_FinLedgerOfficeMapping : System.Web.UI.Page
{
            ...
            if (msg.Trim() == "")
            {
                    ...
                    (chkHeadOffice.Checked == true)
                {
                    objdb.ByProcedure("SpFinMapUnMapLedger", new string[] { "flag", "Ledger_ID", "Office_ID", "LedgerChild_IsActive", "LedgerChild_UpdatedBy", "Active" },
                            new string[] { "3", ddlLedgerName.SelectedValue.ToString(), ViewState["Office_ID"].ToString(), "1", ViewState["Emp_ID"].ToString(), "Yes" }, "dataset");
                    ...
                    objdb.ByProcedure("SpFinLedgerTx",
                         ...
                         new string[] { "6", ddlLedgerName.SelectedValue.ToString(), "Closing Balance", LedgerTx_Amount, Month.ToString(), Year.ToString(), FinancialYear.ToString(), ViewState["Office_ID"].ToString(), "1", ViewState["Emp_ID"].ToString(), "0", "Sub Ledger",Convert.ToDateTime(VoucherDate,cult).ToString("yyyy/MM/dd") }, "dataset");
                }
                else
                {
                    objdb.ByProcedure("SpFinMapUnMapLedger", new string[] { "flag", "Ledger_ID", "Office_ID", "LedgerChild_IsActive", "LedgerChild_UpdatedBy", "Active" },
                        new string[] { "3", ddlLedgerName.SelectedValue.ToString(), ViewState["Office_ID"].ToString(), "1", ViewState["Emp_ID"].ToString(), "No" }, "dataset");
                }
        ...
        try
        {
            if (ddlLedgerName.SelectedIndex > 0)
            {
                ...
                if (ds != null)
                {
                    ...
                    {
                        chkboxmappedofc.DataSource = ds.Tables[0];
                        chkboxmappedofc.DataTextField = "Office_Name";
                        chkboxmappedofc.DataValueField = "Office_ID";
                        chkboxmappedofc.DataBind();
                        foreach (ListItem item in chkboxmappedofc.Items)
                        {
    ...
    protected void chkProductionUnit_CheckedChanged(object sender, EventArgs e)
    {
        try
        {
            ShowHideDeleteBtn();
    ...
    protected void chkAllProductionUnit_SelectedIndexChanged(object sender, EventArgs e)
    {

    }
    //protected void chkAllOtherOffice_CheckedChanged(object sender, EventArgs e)
    //{
    //    try
    //    {
    //        ShowHideDeleteBtn();
    ...
    //    }
    //}
    //protected void chkOtherOffice_SelectedIndexChanged(object sender, EventArgs e)
    //{
    //    try
    //    {
    //        ShowHideDeleteBtn();
