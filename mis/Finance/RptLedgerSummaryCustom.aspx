<%@ Page Title="" Language="C#" MasterPageFile="~/mis/MainMaster.master" AutoEventWireup="true" CodeFile="RptLedgerSummaryCustom.aspx.cs" Inherits="mis_Finance_RptLedgerSummaryCustom" %>
                    ...
                    <div class="row Hiderow">
                        <div class="col-md-5">
                            <div class="form-group">
                                ...
                                <br />
                                <asp:CheckBox ID="chkOpeningBal" runat="server" Text="Opening Bal.&nbsp;&nbsp;" ClientIDMode="Static" />
                                <asp:CheckBox ID="chkDebitAmt" runat="server" Text="&nbsp;Debit Amt.&nbsp;&nbsp;" ClientIDMode="Static" />
                                <asp:CheckBox ID="chkCreditAmt" runat="server" Text="Credit Amt.&nbsp;&nbsp;" ClientIDMode="Static" />
                                <asp:CheckBox ID="chkClosingBal" runat="server" Text="Closing Bal.&nbsp;&nbsp;" ClientIDMode="Static" />
                    ...
                    <div class="row Hiderow">
                        <div class="col-md-12">
                            <div runat="server" id="divExcel">
                                <div class="form-group">
                                    <input type="button" id="btnHide" onclick="FunHideRow();" value="Hide Row" />
                                    <input type="button" id="btnShow" onclick="FunShowRow();" value="Show Row" />
                                    <asp:Button ID="btnBillByBill" runat="server" CssClass="btn-info Aselect1" Text="Bill Details" OnClick="btnBillByBill_Click" />
                                    <input type="button" id="btnExcel" onclick="tableToExcel('tableData', 'W3C Example Table')" value="Export to Excel" />
                                </div>
                            </div>
                    ...
                    <div id="tableData">
                        <div class="row">
                            <div class="col-md-12">
                                    ...
                                    <tr>
                                        <td colspan="5" style="text-align: center;">
                                            <p class="report-title">
                                                ...
                                                <asp:HiddenField ID="HF_OpeningBalance" runat="server" ClientIDMode="Static" />
                                            </p>
                                        </td>
                                    </tr>
                                </table>
                                ...
                                <asp:GridView ID="GridView4" runat="server" ClientIDMode="Static" AutoGenerateColumns="false" class="table table-hover table-bordered">
                                    <Columns>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentFooter" runat="Server">
    ...
    <script>
        function CalculateClosing() {
            ...
            var OpeningBalance = '<%=HF_OpeningBalance.Value%>';
            if (OpeningBalance == "")
                OpeningBalance = "0";
            ...
            var OpeningBalanceTot = OpeningBalance;

            var inputList = document.querySelectorAll('#DetailGrid tbody input[type="checkbox"]:not(:disabled)');

            var i = 0;

            var k = 1;
            // debugger;
            if (document.getElementById('chkOpeningBal').checked) {
                k = 2;
            }
            var rowcount = $('#DetailGrid tr').length;

            $('#DetailGrid tr').each(function (index) {
                if (j >= k & j != rowcount - 1) {
                    ...
                    if (inputList[i].checked == true) {
                        // $(this).children("td").eq(0).css('background-color', '#000');

                        $(this).children('td, th').css('background-color', 'rgb(187 186 186)');


                        if (document.getElementById('chkClosingBal').checked) {
                            if (document.getElementById('chkDebitAmt').checked) {
                                if (document.getElementById('chkCreditAmt').checked) {
                            ...
                            else if (document.getElementById('chkCreditAmt').checked) {
                                $(this).children("td").eq(6).text('');
                            }
                    ...
                    if (inputList[i].checked == false) {
                        ...
                        var temp = OpeningBalance;
                        var Closetemp = OpeningBalance;

                        var tempTot = OpeningBalanceTot;
                        var ClosetempTot = OpeningBalanceTot;
                        ...
                        if (document.getElementById('chkClosingBal').checked) {
                            ...
                            if (document.getElementById('chkDebitAmt').checked) {
                                ...
                                //Debit = $(this).children("td").eq(5).find('input[type="text"]').val();
                                Debit = $(this).children("td").eq(5).text();

                                if (Debit == "") {
                                    Debit = "0";
                                }
                                DrTotal = parseFloat(parseFloat(DrTotal) + parseFloat(Debit)).toFixed(2);
                                if (document.getElementById('chkCreditAmt').checked) {
                                    // Credit = $(this).children("td").eq(6).find('input[type="text"]').val();
                                    Credit = $(this).children("td").eq(6).text();
                                    if (Credit == "") {
                                        Credit = "0";
                                    }

                                    CrTotal = parseFloat(parseFloat(CrTotal) + parseFloat(Credit)).toFixed(2);

                                    Closetemp = parseFloat(parseFloat(temp) + parseFloat(-Debit) + parseFloat(Credit)).toFixed(2);
                                    ClosetempTot = parseFloat(parseFloat(tempTot) + parseFloat(-Debit) + parseFloat(Credit)).toFixed(2);


                                    OpeningBalance = Closetemp;
                                    if (Closetemp >= 0) {
                                        temp = Closetemp + " Cr";
                                    }
                                    else {
                                        temp = Math.abs(Closetemp) + " Dr";;
                                    }

                                    $(this).children("td").eq(7).text(temp);
                                }
                                else {
                                    Closetemp = parseFloat(parseFloat(temp) + parseFloat(-Debit) + parseFloat(Credit)).toFixed(2);
                                    ClosetempTot = parseFloat(parseFloat(tempTot) + parseFloat(-Debit) + parseFloat(Credit)).toFixed(2);
                                    ...
                                    $(this).children("td").eq(6).text(temp);
                                }
                            }
                            else if (document.getElementById('chkCreditAmt').checked) {
                                //Credit = $(this).children("td").eq(5).find('input[type="text"]').val();
                                Credit = $(this).children("td").eq(5).text();
                                if (Credit == "") {
                                    Credit = "0";
                                }
                                CrTotal = parseFloat(parseFloat(CrTotal) + parseFloat(Credit)).toFixed(2);
                    ...
                    if (document.getElementById('chkClosingBal').checked) {
                        if (document.getElementById('chkDebitAmt').checked) {
                            if (document.getElementById('chkCreditAmt').checked) {
                                ...
                                $(this).children("td").eq(5).text(DrTotal);
                                $(this).children("td").eq(6).text(CrTotal);
                                if (OpeningBalance >= 0) {
                                    temp = OpeningBalance + " Cr";
                                }
                                else {
                                    temp = Math.abs(OpeningBalance) + " Dr";;
                                }
                                ...
                                $(this).children("td").eq(6).text(temp);
                                //$(this).children("td").eq(7).text(OpeningBalance);
                            }
                        }
                        else if (document.getElementById('chkCreditAmt').checked) {

                            $(this).children("td").eq(5).text(CrTotal);
                            if (OpeningBalance >= 0) {
                                temp = OpeningBalance + " Cr";
                            }
                            else {
