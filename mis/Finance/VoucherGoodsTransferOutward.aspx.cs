...
public partial class mis_Finance_VoucherGoodsTransferOutward : System.Web.UI.Page
{
    ...
    protected void btnNarration_Click(object sender, EventArgs e)
    {
        ds = objdb.ByProcedure("SpFinVoucherTx",
                 ...
                 new string[] { "14", "Goods Transfer Outward", ViewState["Office_ID"].ToString() }, "dataset");
        ...
        try
        {
            ...
            if (msg == "")
            {
                ...
                foreach (GridViewRow gvrows in GridViewLedgerDetail.Rows)
                {
                    ...
                    if(Ledger_ID.Text == ddlLedgerCr.SelectedValue.ToString())
                    {
                        Status = 1;
                    }
                }
                if (Status == 0)
                {
                    ViewState["LedgerAmount"] = txtLedgerCr_Amount.Text;
                    ddlItemName.ClearSelection();
                    txtQuantity.Text = "";
                    txtRate.Text = "";
                    txtTotalAmount.Text = txtLedgerCr_Amount.Text;
                    CreateItemTable();
        ...
        try
        {
            ...
            if (msg == "")
            {
                ...
                if (Status == 0)
                {
                    ...
                    string Ledger_Name = ddlLedgerCr.SelectedItem.Text + "&nbsp;&nbsp;<b>(Cur Bal: " + txtCurrentBalanceCr.Text + ")</b>";
                    dt_LedgerTable.Rows.Add(null, ddlLedgerCr.SelectedValue.ToString(), Ledger_Name, txtLedgerCr_Amount.Text);
                    ...
                    txtLedgerDr_Amount.Text = LedgerCreditTotal.ToString();
                    GridViewLedgerDetail.FooterRow.Cells[2].Text = "<b>Total : </b>";
                    GridViewLedgerDetail.FooterRow.Cells[3].Text = "<b>" + LedgerCreditTotal.ToString() + "</b>";
        ...
        try
        {
            ...
            for (int rowIndex = 0; rowIndex < gridRows; rowIndex++)
            {
                ...
                if (lblRowNumber.Text != RowNo.ToString())
                {
                    ...
                    dt_LedgerTableTemp.Rows.Add(lblRowNumber.Text, Ledger_ID.Text, Ledger_Name.Text, LedgerTx_Credit.Text);
                }
            }
            ...
            if (msg == "")
            {
                ...
                if (btnAccept.Text == "Accept" && ViewState["VoucherTx_ID"].ToString() == "0" && Status == 0)
                {
                    ...
                    if (ds != null && ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            {
                                ...
                                new string[] { "0", Ledger_ID, VoucherTx_ID, "Goods Transfer Outward", LedgerTx_Amount, Month.ToString(), Year.ToString(), FinancialYear.ToString(), ViewState["Office_ID"].ToString(), "1", ViewState["Emp_ID"].ToString(), RowNo.ToString(), "Main Ledger", "None" }, "dataset");
                                DataSet dsItem = (DataSet)ViewState["dsItem"];
                                DataSet dsItemTemp = new DataSet();
                                dsItemTemp = dsItem;
                                    ...
                                    if (dsItemTemp.Tables[j].TableName == RowNo.ToString())
                                    {
                                        for (int k = 0; k < dsItemTemp.Tables[j].Rows.Count; k++)
                                        {
                                            ...
                                            objdb.ByProcedure("SpFinItemTx", new string[] { "flag", "VoucherTx_ID", "VoucherTx_Name", "VoucherTx_Type", "Ledger_ID", "Item_id", "Unit_id", "Quantity", "Rate", "Amount", "Office_ID", "ItemTx_FY", "ItemTx_IsActive", "ItemTx_InsertedBy", "ItemTx_OrderBy" },
                                                                             new string[] { "0", VoucherTx_ID, "Goods Transfer Outward", "Goods Transfer Outward", Ledger_ID, Item_ID,Unit_id, Quantity, Rate, Amount, ViewState["Office_ID"].ToString(), FinancialYear.ToString(), ItemTx_IsActive, ViewState["Emp_ID"].ToString(), (j + 1).ToString() }, "dataset");
                                       ...
                                       , new string[] { "4", Item_ID, "0", Quantity, Rate, VoucherTx_ID, "Goods Transfer Outward", VoucherTx_No, ViewState["Office_ID"].ToString(), ViewState["Emp_ID"].ToString(), Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd"), Amount }, "dataset");
                        ...
                        objdb.ByProcedure("SpFinLedgerTx",
                                new string[] { "flag", "Ledger_ID", "VoucherTx_ID", "VoucherTx_Type", "LedgerTx_Amount", "LedgerTx_Month", "LedgerTx_Year", "LedgerTx_FY", "Office_ID", "LedgerTx_IsActive", "LedgerTx_InsertedBy", "LedgerTx_OrderBy", "LedgerTx_Type", "LedgerTx_MaintainType" },
                                new string[] { "0", ddlLedgerDr.SelectedValue.ToString(), VoucherTx_ID, "Goods Transfer Outward", DebitAmount, Month.ToString(), Year.ToString(), FinancialYear.ToString(), ViewState["Office_ID"].ToString(), "1", ViewState["Emp_ID"].ToString(), (Count + 1).ToString(), "Main Ledger", "None" }, "dataset");
                    ...
                    {
                        {
                            ...
                            for (int j = 0; j < dsItemTemp.Tables.Count; j++)
                            {
                                if (dsItemTemp.Tables[j].TableName == RowNo.ToString())
                                {
                                   ...
                                   , new string[] { "4", Item_ID, "0", Quantity, Rate, VoucherTx_ID, "Goods Transfer Outward", VoucherTx_No, ViewState["Office_ID"].ToString(), ViewState["Emp_ID"].ToString(), Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd") }, "dataset");
            ...
            if (ds != null)
            {
                ...
                if (ds.Tables[1].Rows.Count > 0)
                {
                    ...
                    for (int rowIndex = 0; rowIndex < gridRows; rowIndex++)
                    {
                        ...
                        Label Ledger_Name = (Label)GridViewLedgerDetail.Rows[rowIndex].Cells[0].FindControl("Ledger_Name");
                        Label LedgerTx_Credit = (Label)GridViewLedgerDetail.Rows[rowIndex].Cells[0].FindControl("LedgerTx_Credit");
                        dt_LedgerTable.Rows.Add(lblRowNumber.Text, Ledger_ID.Text, Ledger_Name.Text, LedgerTx_Credit.Text);
