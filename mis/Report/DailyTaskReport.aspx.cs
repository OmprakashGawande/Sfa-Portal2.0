...
public partial class mis_Report_DailyTaskReport : System.Web.UI.Page
{
    ...
    protected string GetStatusCss(string status)
    {
        switch (status.ToLower())
        {
            case "task not filled":
                return "btn btn-danger btn-pill btn-sm";  // Red
            case "on leave":
                return "btn btn-warning btn-pill btn-sm"; // Yellow
            default:
                return "btn btn-success btn-pill btn-sm"; // Optional fallback
        }
    }
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
        using (XLWorkbook wb = new XLWorkbook())
        {
            ...
            int colCount = gvNotFilledTasks.Columns.Count; // dynamically get columns count
            ws3.Range(1, 1, 1, colCount).Merge();

            ws3.Cell(2, 1).Value = Label1.Text;
            ws3.Cell(2, 1).Style.Font.Bold = true;
            ws3.Range(2, 1, 2, colCount).Merge();

            AddGridViewToWorksheet(gvNotFilledTasks, ws3, 3);

            ws3.Columns().AdjustToContents();
            ws3.Row(3).Style.Font.Bold = true; // assuming header row is at 3
    ...
    private void AddGridViewToWorksheet(GridView gridView, IXLWorksheet worksheet, int startRow)
    {
        ...
        for (int rowIndex = 0; rowIndex < gridView.Rows.Count; rowIndex++)
        {
            ...
            for (int k = 0; k < exportColumnIndexes.Count; k++)
            {
                ...
                string header = gridView.Columns[colIndex].HeaderText.Trim();
                string text = "";

                // Special handling for Status column to get text from Label control inside TemplateField
                if (header.Equals("Status", StringComparison.OrdinalIgnoreCase))
                {
                    Label lblStatus = row.Cells[colIndex].FindControl("lblStatus") as Label;
                    if (lblStatus != null)
                    {
                        text = lblStatus.Text.Trim();
                    }
                    else
                    {
                        text = HttpUtility.HtmlDecode(row.Cells[colIndex].Text.Trim());
                    }
                ...
                // Handle ProjectName, EmployeeName, TaskAllocatedOrFilled columns
                if (colIndex == 1)
                {
